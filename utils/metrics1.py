...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, MPDIoU=False, feat_h=640, feat_w=640,shape_iou=False, scale=0,
             inner_iou=False,  inner_giou=False, inner_diou=False, inner_ciou=False, inner_eiou=False, inner_siou=False, ratio=0.8, eps=1e-7):
    # Returns Intersection over Union (IoU) of box1(1,4) to box2(n,4)
    ...
    if CIoU or DIoU or GIoU:
       ...
       return iou_inner
    elif inner_ciou or inner_diou or inner_giou:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw ** 2 + ch ** 2 + eps  # convex diagonal squared
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou_inner - (rho2 / c2 + v * alpha)  # CIoU
            return iou_inner - rho2 / c2  # DIoU
        c_area = cw * ch + eps  # convex area
        return iou_inner - (c_area - union) / c_area  # GIoU https://arxiv.org/pdf/1902.09630.pdf
    elif inner_eiou:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (
        ...
        cw2 = cw ** 2 + eps
        ch2 = ch ** 2 + eps
        return iou_inner - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2)
    elif inner_siou:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        s_cw = (b2_x1 + b2_x2 - b1_x1 - b1_x2) * 0.5
        ...
        omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
        shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
        return iou_inner - 0.5 * (distance_cost + shape_cost)
    elif MPDIoU:
